...
from pramagent.quantum.budget import (
    QuantumBudgetLedger,
    QuantumBudgetLedgerBackend,
    QuantumBudgetLimits,
)
from pramagent.quantum.evidence import QuantumExecutionEvidence
...
class GuardedQNode:
    ...
    def __init__(
        ...
        *,
        ...
        budget_limits: QuantumBudgetLimits | None = None,
    ) -> None:
        self._qnode = qnode
        self._armor = armor
        ...
        if (budget_ledger is None) != (budget_limits is None):
            raise ValueError("budget_ledger and budget_limits must be configured together")
    ...
    def _session_spent(self) -> tuple[int, float]:
        ...
        if self._budget_ledger is not None:
            snapshot = self._budget_ledger.snapshot(self._tenant_id, self._session_id)
            return snapshot.shots, snapshot.cost_usd
    ...
    def _audit_quantum_event(self, decision, call_args: dict[str, Any], event: str,
        ...
        if event == "quantum_call_executed" and self._meter_func is None:
            payload["evidence"] = self._execution_evidence(
                decision,
                call_args,
                meta,
                shots_observed=call_args["shots"],
                actual_cost_usd=call_args["est_cost_usd"],
            )
        self.last_event = dict(payload)
        self._armor.audit.append(payload)
    ...
    def _execution_evidence(
        ...
        *,
        ...
        evidence = QuantumExecutionEvidence(
            provider=self._provider_hint or "pennylane",
            backend=call_args["device"],
            execution_id=decision.decision_id,
            workload=getattr(qfunc, "__name__", "qnode"),
            device_kind=call_args["device_kind"],
            circuit_hash=(meta or {}).get("fingerprint", ""),
            shots_requested=int(call_args["shots"]),
            shots_observed=int(shots_observed),
            pricing_model=call_args["pricing_key"] or "unknown",
            estimated_cost_usd=float(call_args["est_cost_usd"]),
    ...
    def _audit_reconciliation(self, decision, call_args: dict[str, Any],
        ...
        reconciliation = None
        reservation_id = (meta or {}).get("reservation_id")
        ...
        if reconciliation is not None:
            payload["session_shots_after"] = reconciliation.after.shots
            payload["session_cost_after_usd"] = reconciliation.after.cost_usd
        self._armor.audit.append(payload)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self._budget_ledger is not None:
            reservation = self._budget_ledger.reserve(
                tenant_id=self._tenant_id,
                session_id=self._session_id,
                circuit_name=getattr(qfunc, "__name__", "qnode"),
                estimated_shots=call_args["shots"],
                estimated_cost_usd=call_args["est_cost_usd"],
                limits=self._budget_limits,
            )
            call_args["session_shots_after"] = reservation.after.shots
            call_args["session_cost_after_usd"] = reservation.after.cost_usd
            if not reservation.allowed:
                self._audit_refused(
                    "quantum_budget_reservation_blocked",
                    reservation.reason,
                    shots=call_args["shots"],
                    device=call_args["device"],
                    kind=call_args["device_kind"],
                    pricing_key=call_args["pricing_key"],
                    wires=call_args["wires"],
                    depth=call_args["depth"],
                    estimated_joules=call_args["estimated_joules"],
                    est_cost_usd=call_args["est_cost_usd"],
                    session_shots_after=reservation.after.shots,
                    session_cost_after_usd=reservation.after.cost_usd,
                    meta=meta,
                )
                raise QuantumBudgetExceeded(reservation.reason)
            reservation_id = reservation.reservation_id
            meta = {**meta, "reservation_id": reservation_id}
        ...
        try:
            ...
            self._audit_quantum_event(
                decision,
                call_args,
                "quantum_execution_uncertain",
                meta,
                reason=(
                    f"execution raised {type(exc).__name__}; estimated budget remains reserved"
                ),
            )
            raise
        ...
        elif self._budget_ledger is not None and reservation_id:
            self._budget_ledger.reconcile(
                reservation_id,
                actual_shots=call_args["shots"],
                actual_cost_usd=call_args["est_cost_usd"],
                limits=self._budget_limits,
            )
